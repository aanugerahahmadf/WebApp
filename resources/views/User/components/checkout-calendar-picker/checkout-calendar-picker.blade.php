...
>
    <div
        ...
        wire:ignore
        wire:key="checkout-calendar-picker.{{ $statePath }}"
        @class([
            'fi-fo-checkout-calendar-picker relative w-full',
    ...
    >
        ...
        >
                    ...
                    <template x-for="(cell, cellIndex) in grid" :key="cell ? cell.dateString : 'empty-' + cellIndex">
                        <div class="flex items-center justify-center">
                            <template x-if="cell !== null">
                                <button
                                    type="button"
                                    @click="!cell.isPast && !cell.disabled ? selectDate(cell) : null"
                                    :disabled="cell.disabled || cell.isPast"
                                    class="flex h-8 w-8 items-center justify-center text-sm bg-transparent transition"
                                    :class="{
                                        'pointer-events-none cursor-not-allowed': cell.isPast || cell.disabled,
                                        'opacity-35 font-normal text-gray-400 dark:text-gray-600': cell.isPast,
                                        'font-bold text-primary-600 dark:text-primary-400': cell.selected && !cell.isPast,
                                        'font-semibold text-primary-600 dark:text-primary-400': cell.isToday && !cell.selected && !cell.isPast,
                                        'font-semibold text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-gray-100': !cell.isToday && !cell.selected && !cell.isPast && !cell.disabled,
                                        'font-normal opacity-30 text-gray-400 dark:text-gray-600': cell.disabled && !cell.isPast,
                                    }"
                                    x-text="cell.day"
                                ></button>
                            </template>
            ...
            {{-- Month picker overlay (4 rows x 3 cols) --}}
            ...
            {{-- Year picker overlay (4 rows x 3 cols) --}}
                ...
                <div class="grid grid-cols-3 gap-1">
                    <template x-for="year in visibleYears" :key="year">
                        <button
                            ...
                            class="flex h-9 items-center justify-center text-xs bg-transparent transition disabled:cursor-not-allowed disabled:opacity-40"
                            :class="[
                                year === viewYear
                                    ? 'font-bold text-primary-600 dark:text-primary-400'
                                    : (isYearDisabled(year)
    ...
    @once
        @push('scripts')
            <script>
                function checkoutCalendarPicker(config) {
                    return {
                        state: null,
                        ...
                        isMonthDisabled(month) {
                            ...
                            if (this.viewYear < todayYear || (this.viewYear === todayYear && month < todayMonth)) return true;
                            if (this.minDateParsed) {
                                const minYear  = this.minDateParsed.getFullYear();
                                const minMonth = this.minDateParsed.getMonth();
                                if (this.viewYear < minYear || (this.viewYear === minYear && month < minMonth)) return true;
                            }
                            return false;
                        },

                        isYearDisabled(year) {
                            if (year < this.today.getFullYear()) return true;
                            if (this.minDateParsed && year < this.minDateParsed.getFullYear()) return true;
                            return false;
                        },
                        ...
                        renderDayGrid() {
                            ...
                            for (let day = 1; day <= daysInMonth; day++) {
                                ...
                                const todayOnly = new Date(this.today.getFullYear(), this.today.getMonth(), this.today.getDate());
                                const dateOnly  = new Date(date.getFullYear(), date.getMonth(), date.getDate());
                                cells.push({
                                    day,
                        ...
                        selectDate(cell) {
                            if (cell.disabled || cell.isPast) return;
